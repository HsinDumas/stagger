...
public class ApiParamTreeUtil {
    ...
    public static List<ApiParam> apiParamToTree(List<ApiParam> apiParamList) {
        ...
        for (ApiParam apiParam : apiParamList) {
            // remove pre of field
            apiParam.setField(apiParam.getField()
     ...
     * information.
     */
    public static ApiMethodReqParam buildMethodReqParam(
        ...
        for (ApiParam param : paramList) {
            if (param.isPathParam()) {
                if (pathReqParamMap.containsKey(param.getField())) {
                    param.setConfigParam(true)
                            .setValue(pathReqParamMap.get(param.getField()).getValue());
                }
                ...
                if (queryReqParamMap.containsKey(param.getField())) {
                    param.setConfigParam(true)
                            .setValue(queryReqParamMap.get(param.getField()).getValue());
                }
        ...
        final Set<String> queryParamSet =
                queryParams.stream().map(ApiParam::getField).collect(Collectors.toSet());
        for (ApiReqParam value : queryReqParamMap.values()) {
            if (queryParamSet.contains(value.getName())) {
                continue;
            }
            final ApiParam apiParam =
                    ApiReqParam.convertToApiParam(value).setQueryParam(true).setId(queryParams.size() + 1);
            queryParams.add(apiParam);
        }

        final Set<String> pathParamSet =
                pathParams.stream().map(ApiParam::getField).collect(Collectors.toSet());
        for (ApiReqParam value : pathReqParamMap.values()) {
            if (pathParamSet.contains(value.getName())) {
                continue;
            }
            final ApiParam apiParam =
                    ApiReqParam.convertToApiParam(value).setPathParam(true).setId(pathParams.size() + 1);
