...
public class HtmlCompressorUtil {
    ...
    public static final String BLOCK_TAGS_MAX = BLOCK_TAGS_MIN
            + ",h1,h2,h3,h4,h5,h6,blockquote,center,dl,fieldset,form,frame,frameset,hr,no"
            + " frames,ol,table,tbody,tr,td,th,tfoot,thead,ul";
     ...
     */
    private static final Pattern COND_COMMENT_PATTERN = Pattern.compile(
     ...
     */
    private static final Pattern INTER_TAG_PATTERN_TAG_TAG =
            Pattern.compile(">\\s+<", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);

    /**
     * Pattern to match spaces between tags and custom markers
     */
    private static final Pattern INTER_TAG_PATTERN_TAG_CUSTOM =
            Pattern.compile(">\\s+%%%~", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);

    /**
     * Pattern to match spaces between custom markers and tags
     */
    private static final Pattern INTER_TAG_PATTERN_CUSTOM_TAG =
            Pattern.compile("~%%%\\s+<", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);

    /**
     * Pattern to match spaces between custom markers
     */
    private static final Pattern INTER_TAG_PATTERN_CUSTOM_CUSTOM =
            Pattern.compile("~%%%\\s+%%%~", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
     ...
     */
    private static final Pattern TAG_END_SPACE_PATTERN =
            Pattern.compile("(<(?:[^>]+?))(?:\\s+?)(/?>)", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);

    /**
     * Pattern to match quoted attributes in tags
     */
    private static final Pattern TAG_QUOTE_PATTERN =
            Pattern.compile("\\s*=\\s*([\"'])([a-z0-9-_]+?)\\1(/?)(?=[^<]*?>)", Pattern.CASE_INSENSITIVE);

    /**
     * Pattern to match PRE tags
     */
    private static final Pattern PRE_PATTERN =
            Pattern.compile("(<pre[^>]*?>)(.*?)(</pre>)", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);

    /**
     * Pattern to match TEXTAREA tags
     */
    private static final Pattern TA_PATTERN =
            Pattern.compile("(<textarea[^>]*?>)(.*?)(</textarea>)", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
     ...
     */
    private static final Pattern TAG_PROPERTY_PATTERN =
     ...
     */
    private static final Pattern STYLE_TYPE_ATTR_PATTERN = Pattern.compile(
     ...
     */
    private static final Pattern FORM_METHOD_ATTR_PATTERN = Pattern.compile(
            "(<form[^>]*)method\\s*=\\s*([\"']*)get\\2([^>]*>)", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);

    /**
     * Pattern to match input type attributes
     */
    private static final Pattern INPUT_TYPE_ATTR_PATTERN = Pattern.compile(
            "(<input[^>]*)type\\s*=\\s*([\"']*)text\\2([^>]*>)", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
     ...
     */
    private static final Pattern EVENT_PATTERN1 = Pattern.compile(
            "(\\son[a-z]+\\s*=\\s*\")([^\"\\\\\\r\\n]*(?:\\\\.[^\"\\\\\\r\\n]*)*)(\")", Pattern.CASE_INSENSITIVE);

    /**
     * Pattern to match event attributes with single quotes
     */
    private static final Pattern EVENT_PATTERN2 = Pattern.compile(
            "(\\son[a-z]+\\s*=\\s*')([^'\\\\\\r\\n]*(?:\\\\.[^'\\\\\\r\\n]*)*)(')", Pattern.CASE_INSENSITIVE);
        ...
        while (matcher.find()) {
            if (matcher.group(2).trim().length() > 0) {
                condCommentBlocks.add(
                        matcher.group(1) + HtmlCompressorUtil.compress(matcher.group(2)) + matcher.group(3));
