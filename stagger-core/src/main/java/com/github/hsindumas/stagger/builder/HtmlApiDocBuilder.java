...
public class HtmlApiDocBuilder {
        ...
        if (config.isAllInOne()) {
            ...
            if (config.isCreateDebugPage()) {
                ...
                builderTemplate.buildAllInOne(
                        apiDocList, config, configBuilder, DocGlobalConstants.DEBUG_PAGE_ALL_TPL, INDEX_HTML);
                ...
                builderTemplate.buildAllInOne(
                        apiDocList, config, configBuilder, DocGlobalConstants.ALL_IN_ONE_HTML_TPL, INDEX_HTML);
            }
            ...
            if (config.isCreateDebugPage()) {
                ...
                buildDoc(
                        ...
                        config,
                        configBuilder,
                        DocGlobalConstants.DEBUG_PAGE_SINGLE_TPL,
                        indexAlias);
                Template mockJs = BeetlTemplateUtil.getByName(DocGlobalConstants.DEBUG_JS_TPL);
                FileUtil.nioWriteFile(
                        mockJs.render(),
                ...
                buildDoc(
                        ...
                        config,
                        configBuilder,
                        DocGlobalConstants.SINGLE_INDEX_HTML_TPL,
                        indexAlias);
            }
            builderTemplate.buildErrorCodeDoc(
                    ...
                    configBuilder,
                    apiDocList,
                    DocGlobalConstants.SINGLE_ERROR_HTML_TPL,
                    ERROR_CODE_HTML,
                    indexAlias);
            builderTemplate.buildDirectoryDataDoc(
                    config, configBuilder, apiDocList, DocGlobalConstants.SINGLE_DICT_HTML_TPL, DICT_HTML, indexAlias);
    ...
    private static void buildDoc(
            DocBuilderTemplate builderTemplate,
        ...
        for (ApiDoc doc : apiDocList) {
            ...
            builderTemplate.buildDoc(
                    apiDocList, config, configBuilder, template, doc.getAlias() + ".html", doc, indexHtml);
            index++;
        }
    }
}
