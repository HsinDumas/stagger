...
import com.github.hsindumas.stagger.model.grpc.GrpcJavaMethod;
import com.github.hsindumas.stagger.model.grpc.ProtoInfo;
import com.github.hsindumas.stagger.model.grpc.proto.EnumDefinition;
import com.github.hsindumas.stagger.model.grpc.proto.EnumValue;
import com.github.hsindumas.stagger.model.grpc.proto.Message;
import com.github.hsindumas.stagger.model.grpc.proto.MessageField;
import com.github.hsindumas.stagger.model.grpc.proto.ProtoJson;
...
public class GRpcDocBuildTemplate implements IDocBuildTemplate<GrpcApiDoc>, IJavadocDocTemplate<GrpcJavaMethod> {
    ...
    private void executeCommand(List<String> command, ProtoInfo protoInfo) {
        try {
            if (!protoInfo.isWinOs()) {
                ...
                Files.setPosixFilePermissions(
                        Paths.get(protoInfo.getProtocPath()), PosixFilePermissions.fromString("rwxr-xr-x"));
                Files.setPosixFilePermissions(
                        Paths.get(protoInfo.getProtocGenDocPath()), PosixFilePermissions.fromString("rwxr-xr-x"));
            }
        } catch (IOException e) {
        ...
        try {
            ...
            StreamGobbler outputGobbler =
                    new StreamGobbler(process.getInputStream(), line -> log.warning("[protoc] " + line));
    ...
    private List<String> buildProtocCommand(Set<String> protoFiles, ProtoInfo protoInfo) {
        ...
        // Set output options
        command.add(
                "--doc_out=" + Paths.get(protoInfo.getTargetJsonDirectoryPath()).toAbsolutePath());
        command.add("--doc_opt=json," + protoInfo.getJsonName());

        // Set the plugin path (using absolute path)
        command.add("--plugin=protoc-gen-doc="
                + Paths.get(protoInfo.getProtocGenDocPath()).toAbsolutePath());
    ...
    private List<ApiParam> processMessage(Message message, int level) {
        ...
        for (MessageField field : message.getFields()) {
            ...
            String fieldType = field.isMap()
                    ? ParamTypeConstants.PARAM_TYPE_MAP
                    : enumValues != null
                            ? ParamTypeConstants.PARAM_TYPE_ENUM + "[" + field.getType() + "]"
                            : "repeated".equals(field.getLabel()) ? "array[" + field.getType() + "]" : field.getType();

            String fieldDesc = field.getDescription();
            if (enumValues != null) {
                fieldDesc +=
                        "<br/>" + enumValues.stream().map(EnumValue::getName).collect(Collectors.joining("<br/>"));
            }
            ...
            ApiParam apiParam = ApiParam.of()
                    ...
                    .setType(fieldType)
                    .setDesc(fieldDesc)
                    .setRequired(!"optional".equals(field.getLabel())
                            || "required".equals(field.getLabel())
