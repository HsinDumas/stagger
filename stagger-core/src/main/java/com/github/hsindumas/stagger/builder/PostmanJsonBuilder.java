...
import com.github.hsindumas.stagger.model.postman.InfoBean;
import com.github.hsindumas.stagger.model.postman.ItemBean;
import com.github.hsindumas.stagger.model.postman.RequestItem;
...
public class PostmanJsonBuilder {
    ...
    private static UrlBean buildUrlBean(ApiMethodDoc apiMethodDoc) {
        ...
        String url =
                Optional.ofNullable(apiMethodDoc.getRequestExample().getUrl()).orElse(apiMethodDoc.getUrl());
        ...
        String serverPath =
                CollectionUtil.isNotEmpty(urlBean.getPath()) ? urlBean.getPath().get(0) : "";
        // Add server path
        if (CollectionUtil.isNotEmpty(urlBean.getPath())
                && StringUtil.isNotEmpty(shortUrl)
