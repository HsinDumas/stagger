...
public abstract class BaseHelper {
     ...
     * @return field value
     */
    protected static String getFieldValueFromMockForJson(
            String subTypeName, Map<String, String> tagsMap, String typeSimpleName) {
        String fieldValue = "";
     ...
     * @return boolean
     */
    protected static boolean isIgnoreCustomField(
            DocJavaField docField, boolean isResp, CustomField customRequestField, CustomField customResponseField) {
        if (Objects.nonNull(customRequestField)
                && JavaClassUtil.isTargetChildClass(
                        docField.getDeclaringClassName(), customRequestField.getOwnerClassName())
                && (customRequestField.isIgnore())
                && !isResp) {
            return true;
        }
        return Objects.nonNull(customResponseField)
                && JavaClassUtil.isTargetChildClass(
                        docField.getDeclaringClassName(), customResponseField.getOwnerClassName())
                && (customResponseField.isIgnore())
                && isResp;
    }
     ...
     *
     */
    protected static FieldJsonAnnotationInfo getFieldJsonAnnotationInfo(
            ProjectDocConfigBuilder projectBuilder,
            DocJavaField docField,
            boolean isResp,
        ...
        if (!methodJsonViewClasses.isEmpty()
                && isResp
                && docField.getAnnotations().isEmpty()) {
        ...
        for (Object annotation : docField.getAnnotations()) {
            ...
            // Handle @JsonView
            if (JavaClassUtil.shouldExcludeFieldFromJsonView(
                    annotation, methodJsonViewClasses, isResp, projectBuilder)) {
                fieldJsonAnnotationInfo.setIgnore(true);
                return fieldJsonAnnotationInfo;
            }
            ...
            if (DocAnnotationConstants.SHORT_JSON_SERIALIZE.equals(annotationName)
                    && DocAnnotationConstants.TO_STRING_SERIALIZER_USING.equals(
                            DocUtil.getAnnotationNamedParameter(annotation, DocAnnotationConstants.USING_PROP))) {
            ...
            if (DocAnnotationConstants.SHORT_JSON_FIELD.equals(annotationName)) {
                if (null != DocUtil.getAnnotationProperty(annotation, DocAnnotationConstants.NAME_PROP)) {
                    ...
                    String fieldName = DocUtil.resolveAnnotationValue(
            ...
            if (DocAnnotationConstants.JSON_FORMAT.equals(annotationName)) {
                fieldJsonAnnotationInfo.setFieldJsonFormatType(DocUtil.processFieldTypeNameByJsonFormat(
                        projectBuilder.getApiConfig().getShowJavaType(),
                        docField.getTypeFullyQualifiedName(),
                        annotation));
                fieldJsonAnnotationInfo.setFieldJsonFormatValue(
                        DocUtil.getJsonFormatString(docField.getJavaField(), annotation));
            }
        }
        return fieldJsonAnnotationInfo;
    }
     ...
     */
    protected static CustomFieldInfo getCustomFieldInfo(
            ProjectDocConfigBuilder projectBuilder,
            DocJavaField docField,
            boolean isResp,
            String simpleName,
        ...
        if (Objects.nonNull(customResponseField)
                && isResp
                && Objects.nonNull(customResponseField.getValue())
        ...
        if (Objects.nonNull(customRequestField)
                && !isResp
                && Objects.nonNull(customRequestField.getValue())
        ...
        // cover comment
        if (Objects.nonNull(customRequestField)
                && StringUtil.isNotEmpty(customRequestField.getDesc())
                && JavaClassUtil.isTargetChildClass(simpleName, customRequestField.getOwnerClassName())
                && !isResp) {
            customFieldInfo.setComment(customRequestField.getDesc());
        }
        if (Objects.nonNull(customResponseField)
                && StringUtil.isNotEmpty(customResponseField.getDesc())
                && JavaClassUtil.isTargetChildClass(simpleName, customResponseField.getOwnerClassName())
                && isResp) {
        ...
        // cover fieldName
        if (Objects.nonNull(customRequestField)
                && StringUtil.isNotEmpty(customRequestField.getReplaceName())
                && JavaClassUtil.isTargetChildClass(simpleName, customRequestField.getOwnerClassName())
                && !isResp) {
            customFieldInfo.setFieldName(customRequestField.getReplaceName());
        }
        if (Objects.nonNull(customResponseField)
                && StringUtil.isNotEmpty(customResponseField.getReplaceName())
                && JavaClassUtil.isTargetChildClass(simpleName, customResponseField.getOwnerClassName())
                && isResp) {
