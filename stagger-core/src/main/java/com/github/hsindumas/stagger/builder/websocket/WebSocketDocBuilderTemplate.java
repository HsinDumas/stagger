...
public class WebSocketDocBuilderTemplate implements IBaseDocBuilderTemplate<WebSocketDoc> {
     ...
     * @return ApiAllData
     */
    public List<WebSocketDoc> getWebSocketApiDoc(
            boolean isAsciidoc, ApiConfig config, JavaProjectBuilder javaProjectBuilder) {
        config.setAdoc(isAsciidoc);
        this.checkAndInit(config, Boolean.TRUE);
        config.setParamsDataToTree(false);
        config.setOutPath(
                config.getOutPath() + DocGlobalConstants.FILE_SEPARATOR + DocGlobalConstants.WEBSOCKET_OUT_DIR);
     ...
     */
    public List<WebSocketDoc> getWebSocketApiDoc(
            boolean isAsciidoc, ApiConfig config, ProjectDocConfigBuilder configBuilder) {
        config.setAdoc(isAsciidoc);
        this.checkAndInit(config, Boolean.TRUE);
        config.setParamsDataToTree(false);
        config.setOutPath(
    ...
    public List<WebSocketDoc> getWebSocketApiDoc(ApiConfig config, JavaProjectBuilder javaProjectBuilder) {
        ...
        IWebSocketDocBuildTemplate<WebSocketDoc> webSocketDocBuildTemplate =
                BuildTemplateFactory.getWebSocketDocBuildTemplate(config.getFramework(), config.getClassLoader());
     ...
     */
    public void buildWebSocketAllInOne(
     ...
     */
    public void buildWebSocketApiDoc(
            List<WebSocketDoc> apiDocList, ApiConfig config, String template, String fileExtension) {
