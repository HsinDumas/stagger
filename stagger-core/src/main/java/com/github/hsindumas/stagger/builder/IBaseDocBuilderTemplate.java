...
import com.github.hsindumas.stagger.model.RevisionLog;
import com.github.hsindumas.stagger.template.IDocBuildTemplate;
import com.github.hsindumas.stagger.template.engine.ClasspathResourceLoader;
import com.github.hsindumas.stagger.template.engine.Resource;
...
public interface IBaseDocBuilderTemplate<T extends IDoc> {
     ...
     * @return ApiAllData
     */
    default List<T> getApiDoc(
            boolean isAsciidoc,
            boolean showJavaType,
            boolean paramsDataToTree,
            ApiConfig config,
        ...
        IDocBuildTemplate<T> docBuildTemplate =
    ...
    default void checkAndInitForGetApiData(ApiConfig config) {
        ...
        if (Objects.nonNull(config.getLanguage())) {
            System.setProperty(
                    DocGlobalConstants.DOC_LANGUAGE, config.getLanguage().getCode());
        } else {
            // default is chinese
    ...
    default Map<String, String> setDirectoryLanguageVariable(ApiConfig config, Template mapper) {
        ...
        if (Objects.nonNull(config.getLanguage())) {
            if (DocLanguage.CHINESE.code.equals(config.getLanguage().getCode())) {
                mapper.binding(
                        TemplateVariable.ERROR_LIST_TITLE.getVariable(), DocGlobalConstants.ERROR_CODE_LIST_CN_TITLE);
                ...
                mapper.binding(
                        TemplateVariable.ERROR_LIST_TITLE.getVariable(), DocGlobalConstants.ERROR_CODE_LIST_EN_TITLE);
     ...
     * empty.
     */
    default void bindingCommonVariable(
            ApiConfig config, JavaProjectBuilder javaProjectBuilder, Template template, boolean apiDocListEmpty) {
        String strTime = DateTimeUtil.long2Str(NOW, DateTimeUtil.DATE_FORMAT_SECOND);
     ...
     */
    default void bindingCommonVariable(
            ApiConfig config, ProjectDocConfigBuilder configBuilder, Template template, boolean apiDocListEmpty) {
        String strTime = DateTimeUtil.long2Str(NOW, DateTimeUtil.DATE_FORMAT_SECOND);
    ...
    default void copyJQueryAndCss(ApiConfig config) {
        ...
        if (HighLightJsConstants.HIGH_LIGHT_CSS_DEFAULT.equals(config.getHighlightStyleLink())) {
            IBaseDocBuilderTemplate.copyJarFile(
                    "css/" + DocGlobalConstants.HIGH_LIGHT_STYLE,
     ...
     * @param javaProjectBuilder javaProjectBuilder
     */
    default void buildErrorCodeDoc(
            ApiConfig config, String template, String outPutFileName, JavaProjectBuilder javaProjectBuilder) {
     ...
     */
    default void buildErrorCodeDoc(
            ApiConfig config, String template, String outPutFileName, ProjectDocConfigBuilder configBuilder) {
     ...
     * @return template
     */
    default Template buildErrorCodeDocTemplate(
            ApiConfig config, String template, JavaProjectBuilder javaProjectBuilder) {
        List<ApiErrorCode> errorCodeList = DocUtil.errorCodeDictToList(config, javaProjectBuilder);
        String strTime = DateTimeUtil.long2Str(NOW, DateTimeUtil.DATE_FORMAT_SECOND);
     ...
     */
    default Template buildErrorCodeDocTemplate(
            ApiConfig config, String template, ProjectDocConfigBuilder configBuilder) {
        List<ApiErrorCode> errorCodeList = DocUtil.errorCodeDictToList(config, configBuilder);
        String strTime = DateTimeUtil.long2Str(NOW, DateTimeUtil.DATE_FORMAT_SECOND);
        Template tpl = BeetlTemplateUtil.getByName(template);
        this.setCssCDN(config, tpl);
