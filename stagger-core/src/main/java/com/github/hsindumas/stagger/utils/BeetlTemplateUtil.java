...
public class BeetlTemplateUtil {
    ...
    private static String renderTemplate(String templateName, Map<String, Object> params) {
        try {
            freemarker.template.Template template =
                    TEMPLATE_CACHE.computeIfAbsent(templateName, BeetlTemplateUtil::compileTemplate);
    ...
    private static freemarker.template.Template compileTemplate(String templateName) {
        try {
            ...
            return new freemarker.template.Template(
                    templateName, new StringReader(converted), FREEMARKER_CONFIGURATION);
        } catch (IOException e) {
    ...
    private static final class BeetlSyntaxConverter {
        ...
        private static final Pattern FOR_STATEMENT_PATTERN =
                Pattern.compile("^([A-Za-z_][A-Za-z0-9_]*)\\s+in\\s+(.+)$", Pattern.DOTALL);
        ...
        private static void appendAssignIfNeeded(String statement, StringBuilder out) {
            ...
            out.append("<#assign ")
                    .append(left)
                    .append("=")
                    .append(convertExpression(right))
                    .append(">");
        }
