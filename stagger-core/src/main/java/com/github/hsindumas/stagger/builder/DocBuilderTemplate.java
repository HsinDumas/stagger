...
public class DocBuilderTemplate implements IBaseDocBuilderTemplate<ApiDoc> {
    ...
    public ApiAllData getApiData(ApiConfig config, JavaProjectBuilder javaProjectBuilder) {
        ...
        apiAllData.setApiDocList(listOfApiData(config, new ProjectDocConfigBuilder(config, javaProjectBuilder)));
    ...
    public ApiAllData getApiData(ApiConfig config, ProjectDocConfigBuilder configBuilder) {
        ...
        apiAllData.setProjectId(DocUtil.generateId(config.getProjectName()));
        apiAllData.setLanguage(config.getLanguage().getCode());
        apiAllData.setApiDocList(listOfApiData(config, configBuilder));
        apiAllData.setErrorCodeList(DocUtil.errorCodeDictToList(config, configBuilder));
        apiAllData.setRevisionLogs(config.getRevisionLogs());
        apiAllData.setApiDocDictList(DocUtil.buildDictionary(config, configBuilder));
        return apiAllData;
    }
    ...
    public void buildAllInOne(
            ...
            String template,
            String outPutFileName) {
        this.buildDoc(apiDocList, config, configBuilder, template, outPutFileName, null, null);
    }
     ...
     * @return Template
     */
    public Template buildAllRenderDocTemplate(
            List<ApiDoc> apiDocList,
            ApiConfig config,
            JavaProjectBuilder javaProjectBuilder,
            String template,
        ...
        boolean onlyHasDefaultGroup =
                apiDocList.stream().allMatch(doc -> Objects.equals(CommonConstants.DEFAULT_GROUP_CODE, doc.getGroup()));
        int codeIndex = 0;
        ...
        if (Objects.nonNull(config.getJmeter())) {
            tpl.binding(
                    TemplateVariable.JMETER_PROMETHEUS_LISTENER.getVariable(),
    ...
    public void buildSearchJs(
            ApiConfig config, JavaProjectBuilder javaProjectBuilder, List<ApiDoc> apiDocList, String template) {
        List<ApiErrorCode> errorCodeList = DocUtil.errorCodeDictToList(config, javaProjectBuilder);
        Template tpl = BeetlTemplateUtil.getByName(template);
        // directory tree
     ...
     */
    public void buildSearchJs(
            ApiConfig config, ProjectDocConfigBuilder configBuilder, List<ApiDoc> apiDocList, String template) {
        List<ApiErrorCode> errorCodeList = DocUtil.errorCodeDictToList(config, configBuilder);
        Template tpl = BeetlTemplateUtil.getByName(template);
        // directory tree
        List<ApiDoc> apiDocs = new ArrayList<>();
    ...
    public void buildErrorCodeDoc(
        ...
        FileUtil.nioWriteFile(
                errorTemplate.render(), config.getOutPath() + DocGlobalConstants.FILE_SEPARATOR + outPutFileName);
    }
    ...
    public void buildDirectoryDataDoc(
        ...
        FileUtil.nioWriteFile(
                mapper.render(), config.getOutPath() + DocGlobalConstants.FILE_SEPARATOR + outPutFileName);
    }
    ...
    public void buildDirectoryDataDoc(
            ...
            String outPutFileName,
            String indexAlias) {
        List<ApiDocDict> directoryList = DocUtil.buildDictionary(config, configBuilder);
        Template mapper = BeetlTemplateUtil.getByName(template);
        String strTime = DateTimeUtil.long2Str(NOW, DateTimeUtil.DATE_FORMAT_SECOND);
        mapper.binding(TemplateVariable.PROJECT_NAME.getVariable(), config.getProjectName());
        String style = config.getStyle();
     ...
     */
    public void buildDirectoryDataDoc(
            ApiConfig config, JavaProjectBuilder javaProjectBuilder, String template, String outPutFileName) {
     ...
     */
    public void buildDirectoryDataDoc(
            ApiConfig config, ProjectDocConfigBuilder configBuilder, String template, String outPutFileName) {
        Template mapper = buildDirectoryDataDocTemplate(config, configBuilder, template);
        FileUtil.nioWriteFile(
                mapper.render(), config.getOutPath() + DocGlobalConstants.FILE_SEPARATOR + outPutFileName);
    }
    ...
    public Template buildDirectoryDataDocTemplate(
            ApiConfig config, JavaProjectBuilder javaProjectBuilder, String template) {
        List<ApiDocDict> directoryList = DocUtil.buildDictionary(config, javaProjectBuilder);
        Template mapper = BeetlTemplateUtil.getByName(template);
        this.setDirectoryLanguageVariable(config, mapper);
        // set css cdn
     ...
     */
    public Template buildDirectoryDataDocTemplate(
            ApiConfig config, ProjectDocConfigBuilder configBuilder, String template) {
        List<ApiDocDict> directoryList = DocUtil.buildDictionary(config, configBuilder);
        Template mapper = BeetlTemplateUtil.getByName(template);
        this.setDirectoryLanguageVariable(config, mapper);
        // set css cdn
     ...
     * @return ApiData list
     */
    private List<ApiDoc> listOfApiData(ApiConfig config, ProjectDocConfigBuilder configBuilder) {
