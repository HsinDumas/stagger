...
public abstract class DocBaseTask extends DefaultTask {
	...
	public void action() {
		...
		if (Objects.nonNull(apiConfig.getRpcConsumerConfig())) {
			...
			if (!rpcConsumerPath.isAbsolute()) {
				apiConfig
					.setRpcConsumerConfig(project.getProjectDir().getPath() + "/" + apiConfig.getRpcConsumerConfig());
			}
		}
	...
	private JavaProjectBuilder buildJavaProjectBuilder(Project project, Set<String> excludes, Set<String> includes) {
		...
		SourceSetUtil.getDefaultMainJava(project).ifPresent(src -> {
	...
	private void loadSourcesDependencies(JavaProjectBuilder javaDocBuilder, Project project, Set<String> excludes,
		...
		Set<ResolvedArtifact> resolvedArtifacts = compileConfiguration.getResolvedConfiguration()
		...
		for (ResolvedArtifact resolvedArtifact : resolvedArtifacts) {
			...
			if (selfModule) {
				...
				moduleArtifact = CustomArtifact.builder()
					.setGroup(version.getId().getGroup())
			...
			if (ArtifactFilterUtil.ignoreArtifact(artifact)
					|| ArtifactFilterUtil.ignoreSpringBootArtifactById(artifact)) {
				continue;
			}
		...
		Set<ComponentArtifactsResult> artifactsResults = project.getDependencies()
		...
		try (JarFile jarFile = new JarFile(artifact.getFile())) {
			for (Enumeration<?> entries = jarFile.entries(); entries.hasMoreElements();) {
				...
				if (name.endsWith(".java") && !name.endsWith("/package-info.java")) {
					javaDocBuilder.addSource(new URL("jar:" + artifact.getFile().toURI().toURL() + "!/" + name));
				}
			}
		}
		catch (Throwable e) {
	...
	private void addModuleSourceTree(JavaProjectBuilder javaDocBuilder, TreeMap<String, Project> allModules,
			String artifactName) {
		Project module = allModules.getOrDefault(artifactName, null);
		if (module != null) {
			String modelSrc = String.join(File.separator, module.getProjectDir().getAbsolutePath(),
					GlobalConstants.SRC_MAIN_JAVA_PATH);
